################################################################################
# This tutorial was created by Jonas Frankel-Bricker and Jen Cruz as Part 1 of the 
# "Basic_R" tutorial.  
...
# Any lines beginning with a hashtag "#" indicate "annotation". 
# Lines starting with "#" are not run as code and serve as text within the script.  
# It is imperative that all code you write is thoroughly annotated so that other 
...
# You are now ready to write new code in an R script. 

# Create a new R script by clicking on the green plus symbol dropdown menu in the 
# top left of the window and select "R Script".
# Click on the blue floppy disk symbol to save your script and give it a concise and informative name.  

# Always begin a script by installing and/or loading the "packages" you will need to run 
# your code.  
# Packages are programs that provide additional "functions" and capabilities to 
# those that already come with R ("base" R).
# Functions are commands that tell R to perform tasks.  
# MOST functions follow the syntax: FunctionName( argument )
...
# You only need to install packages once, however, you must reload the packages with the 
...
# You can name the objects (avoid special characters and spaces).
# You should keep naming concise and informative.
...
# Clicking on it will open a new tab showing the contents of the data frame.  
...
# "Site_Name" contains names of geographic sites.  
# Names should be assigned as a "character" class variable.
...
str( CountMatrix )
# This may seem repetitive but is actually a crucial step. You should be checking that each 
...
# Once the variables in your data frame are properly formatted, summarize the data.
summary( CountMatrix )
...
# Currently, none of the variables in your data frame enable you to directly isolate the rows
# corresponding with these faulty cameras.

# It is often beneficial to assign a unique "identifier" variable associated with 
# each row of values.
...
# Look at the entire data frame to identify the faulty cameras. 
CountMatrix
...
## without manually searching through the data frame.
...
# data frame by implementing "subsetting" or indexing methods.
...
# If overlapping packages are loaded in the same environment, you must define the package you want to 
# source the function using a double-colon "::": PackageName::FunctionName()

# Subset the data frame by removing the faulty cameras using subset() in base R and create a
# new data object.
CountMatrix_Clean <- base::subset( CountMatrix, CountMatrix$Camera_ID != c("Beta_3", "Epsilon_3"))

## OR ##

# Index the data frame to keep only the correct data and create a new data object.
...
# Summarize the new data frame to confirm the data has been properly cleaned.
...
# Save your new data frame as a .csv file in the Path for the "Data" directory using 
