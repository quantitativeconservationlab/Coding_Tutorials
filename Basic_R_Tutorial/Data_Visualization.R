...
# and possible outliers for different groups in our data set.
...
# Note how "color" is assigned as an argument in the aes() function inside the geom_boxplot()
# function so you can change the variables that you are calling within the functions 
# themselves.
...
str(CountMatrix_Combined_Melt$PrettyDate)
...
# Note that you aren't saving the output anywhere. 
# It should just display on your console so that you can check you have done it correctly.

# You can then use this temporary summary and input it into ggplot as follows: 
CountMatrix_Combined_Melt %>%
  group_by( Animal, PrettyDate ) %>%
  summarise( Occupancy = mean( Relative_Occupancy ) ) %>%
...
ggplot( . , 
        ...
        # were used to group our data, which are still stored in our new, unsaved data frame.
