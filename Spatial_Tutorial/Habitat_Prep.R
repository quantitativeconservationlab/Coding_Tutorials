...
spp_df <- read.csv( paste( datadir, "spp_df", sep = ""), header = TRUE )
...
# Well sites is a much small file than the NLCD so it is a lot faster to do this
...
hab_df <- hab.wide %>% rowwise() %>% #add new columns for combined landc over categories:
